...
def resize(image):
    ...
    scale_dict={"area":[0,6.4E+5,10E+5,20E+5],"scale":[100,50,30,15]}
    ...
    # target area after rescaling
    target_area=750000
    
    # calculate scaling factor if 
    # image area is bigger than target 
    if image_area>=target_area: scale_percent = (target_area/image_area)*100
    print("[INFO] image scaling factor: {}".format(scale_percent))
...
def process(filename):
    ...
    print("[INFO] original image size: {}".format(img_origin.shape))
